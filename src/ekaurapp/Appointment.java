...
public final class Appointment {
	 ...
	 * 
	 * @param id
	 *            id of the appointment to be retrieved from the database
	 * @return An object of Appointment class after querying the database for the
	 *         provided id
	...
	public static final ArrayList<Appointment> getTodaysAppointmentsFromDb(String department) {
		...
		try {
			String sql = "SELECT id from Appointment WHERE dttm >= curdate() AND dttm < DATE_ADD(curdate(), INTERVAL 1 DAY) AND department = '"
					+ department + "' AND isPending = 1 ORDER BY dttm ASC";
	...
	public final Boolean isDoctorAvailable() {
		...
		String sql = null;
		try {
			sql = "Select MAX(dttm) from appointment WHERE department = '" + this.getDepartment().getDepartmentString()
					+ "' AND dttm <= '" + sdf.format(this.getDateTime()) + "'";
			ResultSet rs = ExecuteQuery.executeSQL(sql);
			if (rs.next()) {
		...
		try {
			sql = "Select MIN(dttm) from appointment WHERE department = '" + this.getDepartment().getDepartmentString()
					+ "' AND dttm > '" + sdf.format(this.getDateTime()) + "'";
			ResultSet rs = ExecuteQuery.executeSQL(sql);
	...
	/**
	 * Tries to create a new Appointment and put the same into the database subject
	...
	/**
	 * Writes an object of Appointment into the database. Is used by the
	 * makeAppointment method only
	 * 
	 * @param appointment
	 *            The appointment to be saved into the database
	...
	private final boolean putAppointmentIntoDb() {
		...
		try {
			...
			String sql = "INSERT into appointment values(" + "'" + this.getId() + "', " + "'" + this.getPatientEmailId()
					+ "', " + "'" + this.getDoctorEmailId() + "', " + "'" + this.getDepartment().getDepartmentString()
					+ "', " + "'" + this.getDateString() + "', " + "'" + this.getSqlIsPending() + "'" + ")";
	 ...
	 * 
	 * @throws IdNotNullException
	 *             Cannot assign ID to non NULL field. Id is already assigned.
	 */
